...
func ConvertRequest(c context.Context, info convmeta.Meta, target types.RelayFormat, request any) (*RequestResult, error) {
	...
	if from == target {
		return &RequestResult{
			Value: EnsureResponsesReasoningSummary(request, from),
			From:  from,
			To:    target,
		}, nil
	}
...
// EnsureResponsesReasoningSummary 补上 Responses 请求里的 reasoning.summary。
//
// 客户端直接打 /v1/responses 时请求走同协议直通，不像 chat / claude 入站那样
// 经过转换器；而转换器写出 reasoning 参数时总会带上 summary（见
// reasoning.ApplyToOpenAIResponses），直通路径没有这一步。Responses API 只在
// 客户端请求了 summary 时才回吐推理内容，于是同一个客户端打 chat 端点能看到
// 思考过程、打 responses 端点却什么都没有。
//
// 只处理已经是 DTO 的请求：未解析的 JSON 走一遍结构体会丢掉扩展字段，
// 而那正是同协议直通要保住的东西。
func EnsureResponsesReasoningSummary(request any, from types.RelayFormat) any {
	...
	case *dto.OpenAIResponsesRequest:
		responsesRequest = typed
	case dto.OpenAIResponsesRequest:
		responsesRequest = &typed
	default:
		return request
	}
	// 没要求推理，或者已经指定了 summary，都不该动。
	if responsesRequest == nil || responsesRequest.Reasoning == nil {
		return request
	}
	if responsesRequest.Reasoning.Effort == "" || responsesRequest.Reasoning.Summary != "" {
		return request
	}

	intent, _, err := reasoning.FromOpenAIResponses(responsesRequest)
	if err != nil || intent.IsEmpty() {
		return request
	}
	if err := reasoning.ApplyToOpenAIResponses(responsesRequest, intent); err != nil {
		return request
	}
