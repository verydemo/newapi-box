...
// ConvertedThinkingSignature 是跨协议转换产出 thinking 块时的占位签名。
//
// Claude 客户端把签名当作 thinking 块完整性的凭据，没有它就可能整块丢弃，
// 用户端表现为"看不到思考过程"。这里拿不到上游的真实签名，只能放一个
// 合法 base64 的占位值：本地显示没问题；若被回传给官方 Anthropic 会被拒。
const ConvertedThinkingSignature = "bmV3YXBpOmNvbnZlcnRlZC10aGlua2luZw=="
		...
		case responsesOutputTypeReasoning:
			if thinking := reasoningOutputText(&output); thinking != "" {
				claudeResponse.Content = append(claudeResponse.Content, dto.ClaudeMediaMessage{
					...
					Signature: ConvertedThinkingSignature,
				})
			}
